...
# Medical Informatics and Radiology, Erasmus MC, Rotterdam, The Netherland
#
...
# Cross-validation script for multi-atlas appearance segmentation of 
# Rotterdam Scan Study femur MRI scans. The algorithm utilizes multi-modal data:
# fat and water-suppressed MRI data.
...
def main() :
    ...
    # scales used in gaussian scale space
    scales =(1.0,1.6,4.0)
    
    # number of classes in segmentation problem  
    nrclasses = 2
    ...
    # radius for dilation mask used in appearance model   
    radius = [5.0]

    # temp directory location  
    staging_dir = '/scratch/mhansson/staging/cross_val_multiAtlasAppFemur'
    ...
    # create test and train folds for n_folds cross-validation
    cv=cross_validation.KFold(len(CV_img),n_folds=num_folds,random_state=0)

    test_train_list = list(cv) 
    ...
    sourcedata = {
      'scalespace_img':   {
        ...
        # Note that the image data are tuples: water and fat-suppressed scans. 
        'atlas_img':   {
    ...
    while foldnr <= num_folds:
            ...
            train_indices,test_indices = test_train_list[foldnr-1]
            sourcedata_fold= {}
            ...
            network = fastr.Network(id_="Multimodal_FemurSegm")
            ...
            # note that this is a consequence of multi-modal image data. 
            ...
            # Moderate class output by power of 'argument'
            ...
            sinkdata = {'out_seg':      'vfs://fastr_data/hipdata/output/segm_hip_wAppModel_foldnr' + str(foldnr)+'_{sample_id}{ext}',
                        'sink_measure': 'vfs://fastr_data/hipdata/output/dice_hip_wAppModel_foldnr' + str(foldnr)+'_{sample_id}_{cardinality}{ext}'}


            # print network
            print network.draw_network(img_format='svg', draw_dimension=True)
            fastr.log.info('^^^^^^^^^^^^^ Starting execution client.')
#            network.execute(sourcedata_fold, sinkdata, cluster_queue="week",tmpdir=staging_dir)
